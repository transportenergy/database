...
def country_map(path):
    ...
    *eppa5_map.tsv* maps from 'Countries' to EPPA5 regions. Some of these are
    ...
    df = pd.merge(eppa5, gtap, how='outer', left_on='Country',
                  right_on='GTAP region', sort=True) \
           .sort_index('columns')
    ...
    df[list(cols.keys())].rename(columns=cols) \
                         .set_index('ISO')[['EPPA5', 'name']] \
                         .sort_index() \
                         .drop_duplicates() \
                         .to_csv(join(path, 'eppa5.tsv'), sep='\t')
...
def import_data(data_path, metadata_path):
    ...
    for var in sorted(vars.keys()):
        ...
        for sub, qty in quantities:
            log('    %sto %s' % (
                ['%s=%s ' % kv for kv in sub.items()][0] if len(sub) else '',
                qty))
            # Convert to pd.DataFrame, unstack the time dimension to columns,
            # convert the region dimension to a column named 'Region'
            df = da.sel(**sub).to_dataframe()[var].unstack(time_dim) \
                   .reset_index().rename(columns={region_dim: 'region'})
