...
import numpy as np
import yaml
# Define the function used for converting a unit from raw to standard in iTEM3 template
...
def value_conversion(row):
    func = unit_conversion(row["Unit"], top_dict["Other_columns_var_mapping"]["Unit"]["MAPPING"][row["Unit"]])
...
def conversion_layer1(df, top_dict):
    ...
    #  top_dict - raw-data-set-dependent dictionary with mapping rules loaded from mapping_conv_phase1.yaml
    ...
    # Name conversion for some columns to keep as consistent as possible while keeping the raw data set's taxonomy
    if "Other_columns_var_mapping" in  top_dict:
        for field in top_dict["Other_columns_var_mapping"]:
            if field != "Unit":
                df[field] = df[field].apply(lambda x: top_dict["Other_columns_var_mapping"][field][x])
            else:
                df[field] = df[field].apply(lambda x: top_dict["Other_columns_var_mapping"][field]["MAPPING"][x])
    return df
...
df_output = df_output[["Model", "Scenario", "Region", "Variable", "Unit", "Mode", "Technology", "Fuel", "Year", "Value", "Source"]]
